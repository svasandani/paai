/* global paaiBgColors, paaiColor, jQuery, wp, _ */
/**
...
( function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( paaiBgColors ).forEach( function( context ) {
					...
					if ( paaiBgColors[ context ].color ) {
						backgroundColorValue = paaiBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( paaiBgColors[ context ].setting ).get();
					}
					paaiSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( paaiBgColors ).forEach( function( context ) {
			wp.customize( paaiBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					...
					paaiSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		paaiSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				paaiSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function paaiSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = paaiColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function paaiSetRetineLogoVisibility( visible ) {
